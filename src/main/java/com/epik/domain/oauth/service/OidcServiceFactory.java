...
public class OidcServiceFactory {
    ...
    private final Map<SocialProvider, AbstractOidcService> serviceMap;
     ...
     */
    public OidcServiceFactory(List<AbstractOidcService> services) {
        // 빈 Map 생성
        this.serviceMap = new HashMap<>();

        // 각 서비스가 반환하는 providerName(KAKAO, GOOGLE)을 key로 사용하여 매핑 구성
        for (AbstractOidcService service : services) {
            SocialProvider provider = service.getProviderName();
     ...
     */
    public AbstractOidcService getOidcService(SocialProvider provider) {
